...
const gradientId = computed(() =>
  `sparkline-grad-${props.id || Math.random().toString(36).substring(2, 9)}`
)
...
const points = computed(() => {
  ...
  const stepX =
    props.width / (props.data.length - 1)
...
const areaPath = computed(() => {
  ...
  return `
    ${linePath.value}
    L ${last.x},${props.height}
    L ${first.x},${props.height}
    Z
  `
