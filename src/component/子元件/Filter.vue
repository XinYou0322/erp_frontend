<template>
  <div class="filter-bar shadow-level-1">
    ...
    <!--
      【保留原功能】根據父元件傳入的 fields 動態產生篩選欄位。
    ...
    <!-- 【我新增】筆數與更新按鈕放在同一組，避免兩者被拆成上下兩行。 -->
    <div
      v-if="showPageSize || showRefresh"
      class="filter-actions"
    >
      <!-- 【我新增】每頁筆數由父元件傳入，變更後通知父元件重新查詢。 -->
      <select
        v-if="showPageSize"
        :value="pageSize"
      ...
      <!-- 【我新增】更新按鈕只發出 refresh，由父元件決定要重新查詢哪些資料。 -->
      <button
        v-if="showRefresh"
...
const props = defineProps({
  ...
  modelValue: {
    type: Object,
    // 【我修改】改為非必填，讓父元件可以使用明確的多組 v-model。
    default: () => ({}),
  },
  ...
  resetValues: {
    type: Object,
    // 【我修改】明確 props 寫法不一定需要 resetValues。
    default: () => ({}),
  },
...
const emit = defineEmits([
  ...
  // 【我新增】使用者改變每頁筆數時，將新筆數傳回父元件。
  "page-size-changed",

  // 【我新增】使用者按下更新按鈕時通知父元件。
  "refresh",

  // 【我新增】以下事件提供類似 HeadNavBar 的多組 v-model 寫法。
  "update:statusValue",
...
// 【我新增】狀態第一個 option 是「全部狀態」，所以索引需減 1。
...
// 【我新增】供應商第一個 option 是「全部供應商」，所以索引需減 1。
...
function handlePageSizeChange(event) {
  ...
  // 【我修改】新增標準 v-model 事件，寫法與 HeadNavBar 相同。
  emit("update:pageSize", Number(selectedValue));

  // 【保留原功能】避免已使用舊事件名稱的頁面失效。
  emit("page-size-changed", Number(selectedValue));
}

// 【我新增】Filter 不直接呼叫 API，只通知父元件執行更新。
