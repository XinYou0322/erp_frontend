<template>
  <!-- =========================
         ...
         ========================= -->
  <div
    class="p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[var(--surface-container)] border border-[var(--outline)]"
  >
    <div>
      <div
        class="font-bold text-[length:var(--font-heading)] text-[var(--on-surface)]"
      >
        原物料主檔管理
      </div>

      <div
        class="text-[length:var(--font-body)] text-[var(--on-surface-variant)] mt-1"
      >
    ...
    <div class="flex items-center space-x-2 shrink-0">
      ...
      <button
        type="button"
        @click="loadMaterials"
        class="btn-secondary text-[length:var(--font-body)] px-3 py-1.5"
      >
        重新整理
      </button>
      ...
      <button
        ...
        class="btn-primary text-[length:var(--font-body)] px-3 py-1.5 flex items-center space-x-1.5"
      >
        <Plus class="w-4 h-4" />
...
const handleMaterialDisabled = async () => {
  await Promise.all([loadMaterials(), loadMaterialSummary()]);
};
