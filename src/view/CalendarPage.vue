...
import CalendarEventModal from '../components/composite/CalendarEventModal.vue';
import CalendarDayDetailModal from '../components/composite/CalendarDayDetailModal.vue';
...
// 統計數據
const totalEvents = computed(() => calendarStore.events.length);
const pendingEventsCount = computed(() => calendarStore.events.filter((e) => e.status === 'pending').length);
const completedEventsCount = computed(() => calendarStore.events.filter((e) => e.status === 'completed').length);
const highPriorityCount = computed(() => calendarStore.events.filter((e) => e.priority === 'high' && e.status !== 'completed').length);
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800">
      <!-- Title & Context -->
      <div class="space-y-1">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 flex items-center justify-center shadow-lg shadow-cyan-950/40">
            <span class="material-symbols-outlined text-[24px]">calendar_month</span>
          </div>
          <div>
            <h1 class="text-xl font-bold text-white tracking-tight flex items-center gap-2">
              行事曆與營運排程
              <span class="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-xs font-normal">
                {{ totalEvents }} 項排程
              </span>
            </h1>
      ...
      <div class="flex items-center flex-wrap gap-2.5">
        <!-- Date Navigator -->
        <div class="flex items-center bg-slate-950 border border-slate-700/80 rounded-xl p-1 shadow-xs">
          <button
        ...
        <!-- Period Display -->
        <div class="px-3 py-1.5 bg-slate-950 border border-slate-700/80 rounded-xl text-white font-bold text-sm min-w-[120px] text-center font-data-mono">
          {{ calendarStore.currentMonthTitle }}
        </div>

        <!-- View Mode Switcher -->
        <div class="flex items-center bg-slate-950 border border-slate-700/80 rounded-xl p-1">
          <button
            v-for="v in [
            ...
            "
          >
            <span class="material-symbols-outlined text-[15px]">{{ v.icon }}</span>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
      <div class="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-slate-800 text-slate-300 flex items-center justify-center border border-slate-700/60">
          <span class="material-symbols-outlined text-[20px]">event_note</span>
        </div>
        <div>
          <p class="text-[11px] text-slate-400 font-medium">總排程數</p>
          <p class="text-lg font-bold text-white font-data-mono">{{ totalEvents }}</p>
        </div>
      </div>
      ...
      <div class="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-lg font-bold text-amber-400 font-data-mono">{{ pendingEventsCount }}</p>
        </div>
      </div>
      ...
      <div class="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-lg font-bold text-rose-400 font-data-mono">{{ highPriorityCount }}</p>
        </div>
      </div>
      ...
      <div class="p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-lg font-bold text-emerald-400 font-data-mono">{{ completedEventsCount }}</p>
        </div>
      </div>
    </div>

    <!-- Filter & Category Toolbar -->
    <div class="space-y-3 bg-slate-900/40 p-4 rounded-2xl border border-slate-800/80">
      <!-- Search & Status Row -->
      <div class="flex flex-col sm:flex-row items-center justify-between gap-3">
        <!-- Search Input -->
        <div class="relative w-full sm:w-80">
          <span class="material-symbols-outlined absolute left-3 top-2 text-slate-500 text-[18px]">
            search
          </span>
          <input
    ...
    <!-- ================================================================= -->
    <div v-if="calendarStore.currentView === 'month'" class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
      <!-- Weekday Headers -->
      <div class="grid grid-cols-7 border-b border-slate-800 bg-slate-950/80 text-center py-2.5 text-xs font-bold text-slate-400">
        <div v-for="(day, idx) in weekdays" :key="day" :class="idx === 0 || idx === 6 ? 'text-amber-400/80' : ''">
          {{ day }}
        </div>
      </div>

      <!-- 42 Days Calendar Cells Grid -->
      <div class="grid grid-cols-7 auto-rows-fr divide-x divide-y divide-slate-800/80">
        <div
          ...
          :class="[
            cell.isCurrentMonth ? 'bg-slate-900/70 hover:bg-slate-800/50' : 'bg-slate-950/40 text-slate-600',
            cell.isToday ? 'ring-1 ring-inset ring-emerald-500/60 bg-emerald-500/5' : '',
          ]"
        >
          ...
          <div class="space-y-1 flex-1 overflow-hidden">
            <div
              ...
              :class="[
                calendarStore.CATEGORY_MAP[evt.category]?.badgeClass || 'bg-slate-800 text-slate-300 border-slate-700',
                evt.status === 'completed' ? 'opacity-60 line-through' : '',
              ]"
              :title="`${evt.startTime} ${evt.title}`"
            >
              <span class="text-[9px] font-data-mono opacity-80 shrink-0">{{ evt.startTime }}</span>
          ...
          <div v-if="cell.events.length > 0" class="flex items-center gap-0.5 pt-1 mt-auto">
            <span
    ...
    <!-- ================================================================= -->
    <div v-else-if="calendarStore.currentView === 'week'" class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
      <div class="grid grid-cols-1 md:grid-cols-7 divide-y md:divide-y-0 md:divide-x divide-slate-800">
        <div
          v-for="day in calendarStore.weekDays"
          :key="day.dateStr"
          class="p-3 flex flex-col min-h-[360px]"
          :class="day.isToday ? 'bg-emerald-500/5 ring-1 ring-inset ring-emerald-500/30' : 'bg-slate-900/60'"
        >
          <!-- Day Header -->
          <div class="flex items-center justify-between pb-2 mb-2 border-b border-slate-800/80">
            <div>
              ...
              <p class="text-sm font-data-mono font-bold text-white flex items-center gap-1.5">
                {{ day.month + 1 }}/{{ day.day }}
                <span v-if="day.isToday" class="px-1.5 py-0.2 rounded-full bg-emerald-500 text-slate-950 text-[10px] font-extrabold">今日</span>
              </p>
            </div>
            <button
              v-if="canEdit"
          ...
          <div class="space-y-2 flex-1 overflow-y-auto">
            ...
            <div
              ...
              :class="[
                calendarStore.CATEGORY_MAP[evt.category]?.badgeClass,
                evt.status === 'completed' ? 'opacity-60' : '',
              ]"
            >
              <div class="flex items-center justify-between text-[10px] text-slate-400 font-data-mono font-semibold">
                <span>{{ evt.startTime }} - {{ evt.endTime }}</span>
                <span v-if="evt.priority === 'high'" class="text-rose-400 font-bold">高急迫</span>
              </div>
              <h5 class="text-xs font-bold text-white leading-snug">
                {{ evt.title }}
              </h5>
              <p v-if="evt.location" class="text-[11px] text-slate-400 truncate flex items-center gap-0.5">
    ...
    <div v-else-if="calendarStore.currentView === 'day'" class="space-y-4">
      <div class="p-5 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-between">
        <div>
          <h2 class="text-lg font-bold text-white flex items-center gap-2">
            {{ calendarStore.selectedDate }} 當日營運日程
            <span class="px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 text-xs font-normal">
              {{ calendarStore.selectedDateEvents.length }} 個活動
            </span>
          </h2>
          <p class="text-xs text-slate-400">點擊排程卡片可快速進行編輯、標記完成或刪除</p>
        </div>
      ...
      <div class="space-y-3">
        ...
        >
          <span class="material-symbols-outlined text-[48px] text-slate-600">event_busy</span>
        ...
        >
          <div class="space-y-2 flex-1">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="px-2.5 py-1 rounded-lg bg-slate-950 text-emerald-400 font-data-mono font-bold text-xs border border-slate-800">
              ...
              <span
                ...
                :class="calendarStore.STATUS_MAP[evt.status]?.badgeClass"
              >
                {{ calendarStore.STATUS_MAP[evt.status]?.label }}
              </span>
            </div>

            <h3
              class="text-base font-bold text-white"
              :class="evt.status === 'completed' ? 'line-through text-slate-400' : ''"
            >
              {{ evt.title }}
            </h3>

            <p v-if="evt.description" class="text-xs text-slate-400 leading-relaxed max-w-3xl">
              {{ evt.description }}
            </p>

            <div class="flex items-center gap-4 text-xs text-slate-400 flex-wrap pt-1">
              <span v-if="evt.location" class="flex items-center gap-1">
                <span class="material-symbols-outlined text-[14px] text-slate-500">location_on</span>
                {{ evt.location }}
              </span>
              <span v-if="evt.organizer" class="flex items-center gap-1">
                <span class="material-symbols-outlined text-[14px] text-slate-500">person</span>
                發起人：{{ evt.organizer }}
              </span>
              <span v-if="evt.relatedRef" class="font-data-mono px-2 py-0.5 rounded bg-slate-950 border border-slate-800 text-slate-300 text-[11px]">
                單據：{{ evt.relatedRef }}
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span>{{ evt.status === 'completed' ? '重新開啟' : '標記完成' }}</span>
            </button>
    ...
    <div v-else-if="calendarStore.currentView === 'agenda'" class="space-y-4">
      ...
      >
        <span class="material-symbols-outlined text-[48px] text-slate-600">search_off</span>
      ...
      >
        <div class="flex items-start gap-3.5">
          <!-- Date & Time Box -->
          <div class="w-20 p-2 rounded-xl bg-slate-950 border border-slate-800 text-center shrink-0">
          ...
          <div class="space-y-1.5">
            ...
            <p v-if="evt.description" class="text-xs text-slate-400 leading-relaxed line-clamp-2">
              {{ evt.description }}
            </p>
