...
export const useAuthStore = defineStore("auth", () => {
  ...
  function login(user) {
    ...
    recordAuditLog(
      "系統登入",
      "auth",
      `使用者「${user.name}」登入系統成功 (${user.roleName})。`,
      "success",
    );
  }
  ...
  function switchUser(user) {
    ...
    recordAuditLog(
      "切換身分",
      "auth",
      `身分快速切換至「${user.name}」(${user.roleName})。`,
      "success",
    );
  }
  ...
  function toggleUserStatus(id) {
    ...
    if (idx !== -1) {
      ...
      recordAuditLog(
        "變更帳號狀態",
        "permissions",
        `將使用者「${user.name}」狀態設定為：${newStatus === "active" ? "啟用中" : "已停用"}。`,
  ...
  function deleteUser(id) {
    ...
    recordAuditLog(
      "刪除使用者",
      "permissions",
      `刪除了使用者帳號「${target.name}」(${target.email})。`,
      "danger",
    );
  }
