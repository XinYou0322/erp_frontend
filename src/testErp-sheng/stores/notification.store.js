...
const INITIAL_NOTIFICATIONS = [
  {
    id: 'ntf-inv-01',
    title: '生豆庫存水位告急',
    message: '【衣索比亞 耶加雪菲 生豆】當前庫存僅存 8.0 kg，已低於安全庫存警戒線 (20 kg)，建議立即安排採購。',
    type: 'warning',
    category: 'inventory',
  ...
  },
  {
    id: 'ntf-wf-01',
    title: '待簽核：採購進貨單審批',
    message: '單號 PO-2026-0301（宏達咖啡原物料，$18,400）目前停留在「主管審核」階段，待您簽核放行。',
    type: 'info',
    category: 'workflow',
  ...
  },
  {
    id: 'ntf-sup-01',
    title: '採購單已確認到貨',
    message: '佳賀包裝科技供應之【牛皮紙透氣閥夾鏈袋 250g】1,000 PCS 已全數入庫驗收完畢。',
    type: 'success',
    category: 'supplier',
  ...
  },
  {
    ...
    message: '帳號 linda.wang@humanist.coffee 於 13:42 透過非常用 IP (210.68.12.9) 登入系統後台。',
    type: 'danger',
    category: 'security',
  ...
  },
  {
    id: 'ntf-sys-01',
    title: '系統資料自動雲端備份完成',
    message: '每日例行性 ERP 多階 BOM 結構樹、供應商資產與 POS 銷售單據已成功加密快照備份。',
    type: 'info',
    category: 'system',
...
export const useNotificationStore = defineStore('notification', () => {
  ...
  const soundEnabled = ref(
    StorageService.get('notification_sound', true)
  );
  ...
  const filteredNotifications = computed(() => {
    return notifications.value
      .filter((n) => {
        if (activeCategory.value !== 'all' && n.category !== activeCategory.value) {
          return false;
        }
  ...
  const playNotificationChime = () => {
    ...
    try {
      ...
      osc2.frequency.exponentialRampToValueAtTime(415.30, now + 0.12);
  ...
  const triggerSampleAlert = () => {
    const samples = [
      {
        title: '生豆即時降載預警',
        message: '【哥倫比亞 薇拉 水洗豆】烘豆車間連續提領 25kg，庫存即將觸及安全下限！',
        type: 'warning',
        category: 'inventory',
        actionLabel: '檢查庫存明細',
        actionRoute: '/bom',
      ...
      {
        title: 'POS 收銀日結報表產出',
        message: '今日門市營業累計 $32,850 已自動核對完成，營收超越預期目標 115%！',
        type: 'success',
        category: 'system',
        actionLabel: '查看營運儀表板',
      ...
      {
        title: '供應商合約到期提醒',
        message: '「永勝烘焙包材」年度特約供貨合約將於 14 天後到期，請評估是否續約。',
        type: 'info',
        category: 'supplier',
        actionLabel: '檢視供應商資料',
        actionRoute: '/suppliers',
