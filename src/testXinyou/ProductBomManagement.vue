<template>
        ...
        <div>
          <div class="flex items-start justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 flex items-center justify-center text-[#0070ea] font-bold shrink-0">
  ...
  @click="handleOpenEditRecipe(product)"
          ...
          <div class="mt-4 p-3 rounded-xl bg-gray-50/80 border border-gray-100 grid grid-cols-3 gap-2 text-center text-xs font-mono">
            <div>
  ...
  <span
    class="text-[11px] font-bold text-gray-500 uppercase tracking-wider block"
  >
    單杯原物料配比 (BOM 組成)
  </span>
  ...
  >
    ...
    >
      ...
      <div>
        ...
        <span
          class="ml-2 text-[10px] text-gray-400 font-mono"
        >
          {{ bom.materialCode }}
        </span>
      </div>
  ...
  <div
    v-else
    class="text-gray-400 text-[11px] py-2 text-center"
  >
    尚未建立標準配方
  </div>
        ...
        <div class="pt-3 border-t border-gray-100/80 text-[11px] text-gray-500 flex items-center justify-between">
          ...
          <span class="px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 text-[10px] font-bold shrink-0 ml-2">{{ product.unit || '杯' }}</span>
        </div>
      </div>
    </div>
...
const categories = ['全部', '純茶類', '奶茶類', '果茶類', '特調類']
